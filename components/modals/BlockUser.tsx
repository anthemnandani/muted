...
const BlockUser = ({
  ...
  const { toggleBlock, isBlockedByMe } = useToggleBlockUser({
    userId,
    isProfile,
    isBlocked,
  });
        ...
        <Card className='rounded-2xl border-none bg-gray-6 shadow-2xl ring-1 ring-gray-7 ring-offset-0'>
          <div className='w-full text-center p-6'>
            ...
            <p className='text-base pt-4 text-white/75 text-center'>
              {isBlockedByMe
                ? `${username} will be able to send you messages, view your ${isThread ? 'threads' : 'posts'}, and follow you. They will not be notified that you unblocked them.`
                : `${username} will not be able to send you messages, see your ${isThread ? 'threads' : 'posts'}, or find your profile. They will not be notified that you blocked them.`}
            </p>
          </div>
