  ...
  return (
    imageUrl && (
      <div className='fixed inset-0 z-[150] bg-black data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0'>
        <span
          onClick={() => setImageUrl('')}
          className='top-5 left-5 text-[#525151] text-[1.2rem] md:text-[1.5rem] absolute p-1 md:p-2 font-thin rounded-full bg-gray-6  cursor-pointer'
        >
          <X className='size-8' />
        </span>
        <div className='fixed left-1/2 top-1/2 z-[999] grid w-full -translate-x-1/2 -translate-y-1/2 max-w-[90vw]  max-h-screen'>
          <Image
            width={630}
            height={630}
            src={imageUrl}
